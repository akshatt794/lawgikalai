...
async function osIndexDoc(id, body) {
  ...
  const url = `${OS_URL.replace(/\/$/, '')}/${OS_INDEX}/_doc/${id}?refresh=true`;
  const headers = { 'Content-Type': 'application/json' };
  if (OS_AUTH) headers['Authorization'] = 'Basic ' + Buffer.from(OS_AUTH).toString('base64');
...
async function osSearch(q, filter) {
  ...
  const must = [{ simple_query_string: { query: q, fields: ['title^2', 'fullText'] } }];
  ...
  const body = {
    query: { bool: { must } },
    highlight: { fields: { fullText: { fragment_size: 140, number_of_fragments: 3 } } },
    size: filter.size || 20
  };
...
const upsertSchema = Joi.object({
  complex: Joi.string().valid(...Object.keys(COMPLEX_ZONES)).required(),
  zone: Joi.string().required(),
  category: Joi.string().valid('JUDGES_LIST', 'JUDGES_ON_LEAVE', 'BAIL_ROSTER', 'DUTY_MAGISTRATE_ROSTER').required(),
...
async function parsePdfFromBuffer(buf) {
  ...
  const { fullText, pages: pageObjs } = splitPages(pages.length ? pages : [data.text || '']);
...
router.get('/summary', async (req, res) => {
  try {
    const agg = await DdcDoc.aggregate([
      {
        $group: {
          _id: { complex: '$complex', zone: '$zone', category: '$category' },
          latestDocDate: { $max: '$docDate' },
          latestUpdatedAt: { $max: '$updatedAt' },
          count: { $sum: 1 }
        }
      },
      { $project: { _id: 0, complex: '$_id.complex', zone: '$_id.zone', category: '$_id.category', latestDocDate: 1, latestUpdatedAt: 1, count: 1 } },
...
router.get('/docs', async (req, res) => {
  try {
    const { complex, zone, category, from, to, page = 1, limit = 20 } = req.query;
    const q = {};
    ...
    const [rows, total] = await Promise.all([
      DdcDoc.find(q).sort({ docDate: -1, updatedAt: -1 }).skip(skip).limit(Number(limit)).lean(),
...
router.post('/docs/upload', upload.single('file'), async (req, res) => {
  try {
    ...
    if (!fitsZone(value.complex, value.zone)) {
      return res.status(400).json({ ok: false, error: `${value.zone} is not a valid zone for ${value.complex}` });
    }
    ...
    if (req.file?.buffer) buf = req.file.buffer;
      ...
      const { data } = await axios.get(value.remoteUrl, { responseType: 'arraybuffer' });
      buf = Buffer.from(data);
      if (!value.s3Url) value.s3Url = value.remoteUrl;
    } else if (value.s3Url) {
      const { data } = await axios.get(value.s3Url, { responseType: 'arraybuffer' });
      buf = Buffer.from(data);
    } else {
      return res.status(400).json({ ok: false, error: 'Provide file, remoteUrl or s3Url' });
    }
    ...
    let doc = await DdcDoc.findOneAndUpdate(key, { $set: payload }, { upsert: true, new: true, setDefaultsOnInsert: true });
...
router.get('/search', async (req, res) => {
  try {
    ...
    if (OS_URL) {
      const data = await osSearch(q, { complex, zone, category, size: Number(size) || 20 });
    ...
    const rows = await DdcDoc
      .find({ $text: { $search: q }, ...filter })
      .select({ score: { $meta: 'textScore' }, complex: 1, zone: 1, category: 1, title: 1, docDate: 1, s3Url: 1, fullText: 1 })
      .sort({ score: { $meta: 'textScore' } })
      .limit(Number(size) || 20)
      .lean();

    // basic highlights
    const rgx = new RegExp(`(.{0,70})(${q.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')})(.{0,70})`, 'i');
    const hits = rows.map((r) => {
      const m = r.fullText?.match(rgx);
      const snippet = m ? `${m[1]}${m[2]}${m[3]}` : (r.fullText || '').slice(0, 140);
      return {
        id: String(r._id),
        score: r.score,
        complex: r.complex,
...
router.get('/lookup', async (req, res) => {
  ...
  try {
    const { data } = await axios.get(`${req.protocol}://${req.get('host')}${req.baseUrl}/search`, { params: req.query });
