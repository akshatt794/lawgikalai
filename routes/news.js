...
const UPLOAD_DIR =
  process.env.NODE_ENV === "production" ? "/tmp/uploads/news" : "uploads/news";
...
router.post("/upload", upload.single("image"), async (req, res) => {
  ...
  try {
    ...
    if (req.file) {
      ...
      const key = `${S3_PREFIX}/${Date.now()}_${Math.random()
        .toString(36)
        .slice(2)}${ext}`;
    ...
    if (imageUrl) {
      data.image = { url: imageUrl }; // always consistent format
    }
    ...
    return res.json({
      message: "News uploaded with extended fields!",
      news: orderedNews,
    });
  } catch (err) {
...
router.get("/related/:category", async (req, res) => {
  try {
    ...
    const relatedNews = await News.find({ category })
      .limit(6)
      .sort({ createdAt: -1 });
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch related news", details: err.message });
  }
});
...
router.post("/toggle-save", auth, async (req, res) => {
  try {
    ...
    if (!newsId)
      return res.status(400).json({ error: "Missing newsId in request body" });
    ...
    const savedNewsStringList = user.savedNews
      .map((id) => id?.toString())
      .filter(Boolean);
